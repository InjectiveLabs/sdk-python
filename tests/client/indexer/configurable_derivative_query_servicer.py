...
class ConfigurableDerivativeQueryServicer(exchange_derivative_grpc.InjectiveDerivativeExchangeRPCServicer):
    def __init__(self):
        ...
        self.stream_positions_v2_responses = deque()
    ...
    async def OpenInterest(self, request: exchange_derivative_pb.OpenInterestRequest, context=None, metadata=None):
    ...
    async def StreamPositionsV2(
        self, request: exchange_derivative_pb.StreamPositionsV2Request, context=None, metadata=None
    ):
        for event in self.stream_positions_v2_responses:
            yield event
