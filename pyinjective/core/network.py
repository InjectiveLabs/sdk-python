...
from typing import Tuple, Optional, Coroutine
...
class CookieAssistant(ABC):
    ...
    @abstractmethod
    async def chain_cookie(self, metadata_query: Coroutine) -> str:
        ...

    @abstractmethod
    async def exchange_cookie(self, metadata_query: Coroutine) -> str:
        ...

    async def chain_metadata(self, metadata_query: Coroutine) -> Tuple[Tuple[str, str]]:
        cookie = await self.chain_cookie(metadata_query=metadata_query)
        return (("cookie", cookie),)

    async def exchange_metadata(self, metadata_query: Coroutine) -> Tuple[Tuple[str, str]]:
        cookie = await self.exchange_cookie(metadata_query=metadata_query)
        return (("cookie", cookie),)
...
class MainnetKubernetesCookieAssistant(CookieAssistant):
    ...
    async def chain_cookie(self, metadata_query: Coroutine) -> str:
        if self._chain_cookie == None:
            async with self._chain_cookie_initialization_lock:
                if self._chain_cookie == None:
                    await self._fetch_chain_cookie(metadata_query=metadata_query)
    ...
    async def exchange_cookie(self, metadata_query: Coroutine) -> str:
        if self._exchange_cookie == None:
            async with self._exchange_cookie_initialization_lock:
                if self._exchange_cookie == None:
                    await self._fetch_exchange_cookie(metadata_query=metadata_query)
        cookie = self._exchange_cookie
        self._check_exchange_cookie_expiration()

        return cookie

    async def _fetch_chain_cookie(self, metadata_query: Coroutine):
        metadata = await metadata_query
    ...
    async def _fetch_exchange_cookie(self, metadata_query: Coroutine):
        metadata = await metadata_query
        cookie_info = next((value for key, value in metadata if key == "set-cookie"), None)
